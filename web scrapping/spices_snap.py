...
BASE_URL = "https://www.snapdeal.com/search?keyword=cooking-oil"  # Change this for other categories
KEYWORD = BASE_URL.split("keyword=")[-1].split("&")[0].capitalize()
TARGET_PRODUCT_COUNT = 400 # Adjust for category size
...
async def extract_product_data(page, url, retry=2):
    try:
        ...
        speciality = specs.get("speciality", "") or specs.get("feature", "") or specs.get("speciality/feature", "")
        ...
        return {
    "title": title or "",
    "type": "Cooking Oil",  # fixed main category
    "subtype": type_value or "Namkeen",  # extracted from spec key 'type'
...
async def main():
    async with async_playwright() as p:
        ...
        all_data = []
        for url in tqdm(product_links, desc=f"🍽️ Scraping {KEYWORD} Products"):
